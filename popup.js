...
async function OnLoad() {
	...
	if (savedRecordings && savedRecordings.length > 0) {
		...
		for (var [index, recording] of savedRecordings.entries()) {
			...
			var audio = $(`<option>Audio ${index + 1}&nbsp;&nbsp;(${durations[index]} seconds)</option>`);
	...
	if (unsavedRecording) {
		...
		var audio = $(`<option>Audio ${index}&nbsp;&nbsp;(${durations[index - 1]} seconds)</option>`);
		$("#slcRecordings").append(audio);
		$("#btnUploadAudio").removeClass("disabled");
	...
	selections = await SessionData.get("selections");
...
async function Login() {
	...
	$.ajax({
		...
		success: async function (data) {
			if (!data.error) {
				// await SessionData.set("token", data.token);
...
async function Record() {
	...
	chrome.tabCapture.capture({ audio: true, video: false }, (stream) => {
		...
		recorder.onstop = async (e) => {
			...
			await SessionData.set("recordings", recordings);
			await SessionData.set("durations", durations);
			var audio = $(`<option>Audio ${$("#slcRecordings option").length + 1}&nbsp;&nbsp;(${duration} seconds)</option>`);
			$("#slcRecordings").append(audio);
...
async function AudioDescription() {
	await SessionData.set("recordingDescription", $("#txtAudioDescription").val());
}
...
// 	console.log('recordings>>>>>>>0000000', recordings);

// 	$.ajax({
// 		type: "POST", url: `${API_URL}/ext/upload-audio`, dataType: "json",
// 		async: false, contentType: 'application/json',
// 		data: JSON.stringify({ title, description, pageDate, date, url, recordings }),
// 		beforeSend: function (xhr) {
// 			xhr.setRequestHeader("Authorization", "bearer " + token);
// 		},
// 		success: async function (x) {
...
// 		const filename = `recording_${Math.floor(Math.random() * 100000000)}.mp3`;
...
async function UploadAudio() {
	...
	for (let i = 0; i < recordings.length; i++) {
		...
		// Convert the audio data to a binary format
		const audioBlob = new Blob([recording], { type: 'audio/wav' });
		const audioBuffer = await audioBlob.arrayBuffer();
		const audioData = new Uint8Array(audioBuffer);

		// Encode the audio data as an MP3 file
		const mp3Encoder = new lamejs.Mp3Encoder(1, 44100, 128);
		const mp3Data = mp3Encoder.encodeBuffer(audioData);
		const mp3EndData = mp3Encoder.flush();

		// Create a Blob object for the MP3 file
		const mp3Blob = new Blob([mp3Data, mp3EndData], { type: mimeType });

		formData.append('recording', mp3Blob, filename);
	}
...
// 		// Create a Blob object for the MP3 file
// 		const mp3Blob = new Blob([].concat(...mp3Data, mp3EndData), { type: mimeType });
...
// 		let bf = Buffer.from(recording, "base64")


// 		formData.append('recording', bf, filename)
// 	}
...
async function UploadAudio() {
	...
	formData.append('recording', recordings);
...
async function AddSelection(text, color, percentage, id) {
	...
	if (text) {
		...
		var selection = $(`<li data-id="${id}"><label class="color-icon" style="background-color: ${color};">&nbsp;&nbsp;</label>
